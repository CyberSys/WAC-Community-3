...
function ENT:AddBackRotor()
	if self.UsePhysRotor then
		self.BackRotor = self:addEntity("prop_physics")
		self.BackRotor:SetModel("models/props_junk/sawblade001a.mdl")
		self.BackRotor:SetPos(self:LocalToWorld(self.TopRotorPos))
		self.BackRotor:SetAngles(self:GetAngles())
		self.BackRotor:SetOwner(self.Owner)
		self.BackRotor:SetNotSolid(true)
		self.BackRotor:Spawn()
		self.BackRotor.Phys = self.BackRotor:GetPhysicsObject()
		self.BackRotor.Phys:EnableGravity(false)
		self.BackRotor.Phys:SetMass(5)
		self.BackRotor.Phys:EnableDrag(false)
		self.BackRotor:SetNoDraw(true)
		self.BackRotor.fHealth = 100
		self.BackRotor.wac_ignore = true
		if self.BackRotorModel then
			local e = self:addEntity("wac_hitdetector")
			...
			e:Spawn()
			e:SetNotSolid(true)
			e:SetParent(self.BackRotor)
			e.wac_ignore = true
			local obb=e:OBBMaxs()
			self.BackRotorWidth=(obb.x>obb.y and obb.x or obb.y)
			self.BackRotorHeight=obb.z
			self.BackRotorModelN=self.BackRotorModel
			self.BackRotorModel=e
		end
...
function ENT:Think()
	...
	if !self.disabled then
		if self.UpdateSecond<crt then
			...
			if self.Smoke2 then
				self.Smoke2:SetKeyValue("renderamt", tostring(math.Clamp(self.rotorRpm*170, 0, 200)))
				self.Smoke2:SetKeyValue("Speed", tostring(50+self.rotorRpm*50))
				self.Smoke2:SetKeyValue("JetLength", tostring(50+self.rotorRpm*50))
			end
			self:UpdateSeats()
	...
	if self.UsePhysRotor then
		if self.TopRotor and self.TopRotor.Phys and self.TopRotor.Phys:IsValid() then
			...
			if IsValid(self.BackRotor) and self.BackRotor.Phys:IsValid() then
				-- back rotor physics
				local backrotor = {}
				backrotor.phys = self.BackRotor.Phys
				backrotor.angVel = backrotor.phys:GetAngleVelocity()
				backrotor.upvel = self.BackRotor:WorldToLocal(self.BackRotor:GetVelocity()+self.BackRotor:GetPos()).z
				backrotor.brake =
					(math.abs(self.rotateX) + math.abs(self.rotateY) + math.abs(self.rotateZ))*0.01
					+ math.Clamp(math.abs(backrotor.angVel.z) - 2950, 0, 100)/10 -- RPM cap
					+ math.pow(math.Clamp(1500 - math.abs(backrotor.angVel.z), 0, 1500)/900, 3)
					+ math.abs(backrotor.angVel.z/10000)
					- (backrotor.upvel - self.rotorRpm)*self.upMul/1000

				backrotor.targetAngVel =
					Vector(0, 0, math.pow(self.engineRpm,2)*self.BackRotorDir*10)
					- backrotor.angVel*backrotor.brake/200

				backrotor.phys:AddAngleVelocity(backrotor.targetAngVel)
			end
			...
			if not (IsValid(self.BackRotor) and self.BackRotor.Phys:IsValid()) then
...
function ENT:DamageSmallRotor(amt)
	...
	if self.BackRotor and self.BackRotor:IsValid() then
		...
		self.BackRotor.Phys:AddAngleVelocity((self.BackRotor.Phys:GetAngleVelocity()*-amt)*0.001)
		if self.BackRotor.fHealth < 0 then
			self:KillBackRotor()
			if !self.Sound.CrashAlarm:IsPlaying() and !self.disabled then
				self.Sound.CrashAlarm:Play()
			end
		elseif self.BackRotor.fHealth < 50 and !self.Sound.MinorAlarm:IsPlaying() and !self.disabled then
...
function ENT:KillBackRotor()
	...
	local e = self:addEntity("prop_physics")
	e:SetModel(self.BackRotorModelN)
	...
	if ph:IsValid() then
		ph:SetMass(1000)
		ph:EnableDrag(false)
		ph:AddAngleVelocity(self.BackRotor.Phys:GetAngleVelocity())
		ph:SetVelocity(self.BackRotor.Phys:GetAngleVelocity():Length()*self.BackRotor:GetUp()*0.5 + self.BackRotor:GetVelocity())
	end
...
function ENT:KillTopRotor()
	...
	self:SetOn(false)
	if self.BackRotor and self.Coaxial then self:KillBackRotor() end
end
--[###] Rotor Damage
...
function ENT:DamageEngine(amt)
	...
	if self.engineHealth < 80  then
		...
		if !self.Smoke and self.engineHealth>0 then
			self.Smoke = self:CreateSmoke(self.SmokePos)
			if self.SmokePos2 then self.Smoke2 = self:CreateSmoke(self.SmokePos2) end
		end
		...
		if self.engineHealth < 50 then
			...
			if self.engineHealth < 20 and !self.EngineFire then
				...
				if self.FirePos2 then
					local fire2 = ents.Create("env_fire_trail")
					fire2:SetPos(self:LocalToWorld(self.FirePos2))
					fire2:Spawn()
					fire2:SetParent(self.Entity)
					self.EngineFire2 = fire2
				end
			end
	...
	if self.Smoke2 then
		local rcol = math.Clamp(self.engineHealth*3.4, 0, 170)
		self.Smoke2:SetKeyValue("rendercolor", rcol.." "..rcol.." "..rcol)
	end
...
function ENT:CreateSmoke(pos)
	...
	smoke:SetPos(self:LocalToWorld(pos))
